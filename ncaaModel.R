...
#' Runs a full simulation model
#' ncaa.dat - properly formulated datafile, exactly as the examples in /data
#' n.sim - the number of simulations to run
#' n.games - the number of games to play to simulate a matchup
#' priors - a matrix of prior averages based on power rankings

full.sim <- function(ncaa.dat,n.sim,n.games,priors){
...
for(x in 1:n.sim){
  ...
  for(i in 1:4){
    ...
    region.out[i,x,,] <- region.sim(bracket.dat,priors)
  }
  ...
  champ.out <- champion.sim(champ.dat,priors)
  cat(x,"\n")
  for(i in 1:4){
...
#write.csv(combined.output,"data/results.csv")


### Pretty output
clust <- c(2,4,8,16,32,64)
pout <- matrix(NA,ncol=6,nrow=32)
for(i in 1:6){
  m <- matrix(1:64,nrow=clust[i])
  for(j in 1:dim(m)[2]){
    pout[j,i] <- as.character(combined.output[m[which.max(combined.output[m[,j],2]),j],1])
  }
  
}
return(list(fullout = combined.output,pretty = pout))
}


#read data
ncaa.dat <- read.csv("data/ncaa2015.csv")
...
my.priors <- rbind(priors1,priors2)



tourn2015 <- full.sim(ncaa.dat,5000,10,my.priors)
